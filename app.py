...
<head>
    ...
    <style>
        * {
        ...
        .chat-input { flex:1; padding:12px; border-radius:8px; border:1px solid #f0dfe8; }
        .chat-send-btn { padding:10px 14px; border-radius:8px; background:#ff9dbf; color:white; border:none; cursor:pointer; }
        .form-input { padding:8px; border:1px solid #f0dfe8; border-radius:8px; }
    ...
    <div class="legacy-container">
        <header>
            <div class="header-content">
                <div class="logo">
                    <div style="width:44px;height:44px;border-radius:10px;background:linear-gradient(180deg,#fff0f6 0%,#ffe5f0 100%);display:flex;align-items:center;justify-content:center;color:#ff6aa2;font-weight:800;font-size:18px;">WH</div>
                    <h1>(Name of our app)</h1>
                </div>
                <nav>
                    <button class="nav-btn active" onclick="showSection('home')">Home</button>
                    <button class="nav-btn" onclick="showSection('menstrual')">Menstrual</button>
                    <button class="nav-btn" onclick="showSection('fitness')">Diet & Exercise</button>
                    <button class="nav-btn" onclick="showSection('chatbot')">Assistant</button>
                </nav>
            </div>
        </header>
...
def chatbot():
    ...
    # attempt to use local rag wrapper if available
    try:
        from chat.rag import get_rag
        rag = get_rag()
        # try load silently (will prefer LOCAL_LLM_PATH env var if set)
        rag.load()
        if not rag.is_ready():
            return {"status": "error", "message": "Local model not available. See server logs or set LOCAL_LLM_PATH."}
        out = rag.generate(prompt or 'Hello')
        return {"status": "success", "response": out}
    except Exception as e:
        return {"status": "error", "message": f"chatbot error: {e}"}
